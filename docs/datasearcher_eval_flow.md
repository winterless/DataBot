...
| **generate_download_list** | `MODE=generate_download_list ./smoke_datasearcher.sh` | sample_scored | `download_list.jsonl` | 从 sample_scored 生成 download_list（top200+2-200GB），可手动筛选后再 download |
| **download** | `MODE=download ./smoke_datasearcher.sh` | download_list.jsonl | `data/hf__*/` | 全量下载，需先有 download_list.jsonl；默认最多 4 个 repo 并行，重复执行会调用 HF Hub 补齐缺失文件 |

各司其职：download 不跑 search/slice/eval。推荐流程：eval → generate_download_list → 手动筛选 download_list.jsonl → download。下载中断后可直接重跑 `MODE=download` 继续补齐。

下载并发配置：`configs/datasearcher_api.json` 中 `download.max_workers` 默认是 `4`，表示最多同时下载 4 个 repo；单个 repo 卡住时不会阻塞其余 worker。
